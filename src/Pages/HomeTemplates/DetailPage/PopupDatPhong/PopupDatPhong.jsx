...
export default function PopupDatPhong({gia_tien}) {
    ...
    const [form, setForm] = useState({
        ...
        ngay_di: "",
        so_luong_khach: 0,
        user_id: JSON.parse(localStorage.getItem(USER_LOGIN))?.user_id
    })

    const startDate = new Date(form.ngay_den);
    const endDate = new Date(form.ngay_di);
                ...
                <div className="bg-white shadow-xl border rounded-xl p-6 w-full lg:w-5/6 mx-auto">
                    <div className="relative w-full">
                        <div className="hidden md:flex justify-between items-center mb-4">
                            <div>
                                <span>$</span>
                                <span className="text-xl font-semibold">{gia_tien}</span>
                        ...
                        <div className="flex flex-col border border-solid border-gray-400 rounded-md">
                            <div className="flex w-full border-b border-solid border-gray-400">
                                <div className="border-r border-solid border-gray-400 rounded-tl-md w-full p-2 cursor-pointer hover:bg-gray-100"><div className="text-xs uppercase font-semibold">Nhận phòng</div>
                                    <div className="m-1">
                                        <input type="date" name="ngay_den" onInput={handleChange} />
                                    </div>
                                </div>
                                <div className=" rounded-tr-md w-full p-2 cursor-pointer hover:bg-gray-100">
                                    ...
                                    <div className="m-1">
                                        <input type="date" name="ngay_di" onInput={handleChange} />
                                    </div>
                                </div>
                            </div>
                            <div className="p-2">
                                ...
                                <div className="flex justify-between items-center m-1">
                                    <button type='button' className="w-8 h-8 bg-gray-300 hover:bg-red-400 duration-200 rounded-xl text-white cursor-pointer" onClick={() => {
                                        if (form.so_luong_khach > 0) {
                                            form.so_luong_khach = form.so_luong_khach - 1;
                                            setForm({
                                                ...form,
                                                so_luong_khach: form.so_luong_khach
                                            });
                                        }
                                    }}>-</button>
                                    <div><span>{form.so_luong_khach}</span> khách</div>
                                    <button type='button' className="w-8 h-8 bg-gray-300 hover:bg-red-400 duration-200 rounded-xl text-white cursor-pointer" onClick={() => {
                                        form.so_luong_khach = form.so_luong_khach +1;
                                        setForm({
                                            ...form,
                        ...
                        <div className="border-b py-2">
                            <div className="flex justify-between py-1 text-base">
                                <div className="underline text-gray-600">$ {gia_tien} x {amount} đêm</div>
                                <div>
                                    <span>{gia_tien * amount}</span> $</div>
                            </div>
                            <div className="flex justify-between py-1 text-base">
                                <div className="underline text-gray-600">Phí dịch vụ</div>
                                <div>
                                    <span>{gia_tien * amount * 2 / 100}</span> $</div>
                            </div>
                        </div>
                        <div className="flex justify-between items-center text-lg font-semibold pt-3">
                            <div>Tổng trước thuế</div>
                            <div>{gia_tien * amount + gia_tien * amount * 2 / 100} $</div>
                        </div>
                    </div>
                </div>
            </form>
