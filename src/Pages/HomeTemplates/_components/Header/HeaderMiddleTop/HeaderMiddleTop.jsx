...
export default function HeaderMiddleTop() {
    ...
    const renderTenViTri = () => {
        return data?.map((item, index) => {
            return <option key={index} value={item.vitri_id} className='p-3 hover:bg-pink-200 text-md cursor-pointer transition-all duration-300 hover:text-pink-600'>{item.ten_vi_tri}</option>
        })
    }
