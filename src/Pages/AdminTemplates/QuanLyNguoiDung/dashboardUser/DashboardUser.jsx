    ...
    const columns = [
        ...
        },
        {
            ...
            render: (words, nguoiDung) => {
                return <Fragment>
                    <NavLink key={1} to={`/admin/quanlynguoidung/edit-user/${nguoiDung.user_id}`}>
                    ...
                    <span className='cursor-pointer' key={6} onClick={() => {
                        ...
                        if (window.confirm(`Bạn chắc chắn xóa người dùng này ${nguoiDung.name} này!?`)) {
                            ...
                            dispatch(actFetchDeleteUser(Number(nguoiDung.user_id)));
                        }
                    }}>
                        <DeleteOutlined className='p-2 text-red-700 text-lg text-right' />
                    </span>
                    <NavLink key={2} to={`/admin/quanlynguoidung/info-user/${nguoiDung.user_id}`} className='navLinkDSND'>
                        <CalendarOutlined />
